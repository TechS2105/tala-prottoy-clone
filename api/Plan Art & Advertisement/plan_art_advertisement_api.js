const plan_art_advertisement_api = [

    // Plan Your Visit
    {

        id: 1,
        page_name: 'Plan Your Visit',
        page_hero_image: '/images/Plan To Visit Image/plan_to_visit_image.jpeg',
        page_hero_image_alttext: 'Plan To Visit',
        page_title: 'Reaching Tala Prattoy',
        reaching_tala_prattoy_details: [

            {

                title: 'By Air',
                para: '10 km away from Netaji Subhas Chandra Bose International Airport, Kolkata.'

            },

            {

                title: 'By Rail',
                para: 'Nearby Railway Stations are Dumdum Junction and Kolkata Station. Both the stations are within 1 km from Tala Prattoy Puja Arena.'

            },

            {

                title: 'By Road',
                para: 'Tala prattoy is well connected with entire part of the city Kolkata. It is situated in northern part of the City Of Joy. Nearby main roads are BT Road, RG Kar Road, Raja Manindra Road and Indra Biswas Road.'

            },

            {

                title: 'By Metro',
                para: 'Nearest Metro station is Belgachia.'

            }

        ]

    },

    // Art & Artist
    {

        id: 2,
        page_name: 'Art & Artist',
        page_hero_image: '/images/Art Image/art_image.jpg',
        page_hero_image_alttext: 'Art & Artist',
        art_artist_details: [

            {

                title: 'Art and Artist',
                para: 'Tala Prattoy, in its journey to encourage and expand Art aspects of Durga Puja, has been acclaimed by all quarters for its essence of festivity with Art. Eminent artists with their creations projected the feel of the humanity and time in different years. And there have always been aesthetically expressed a vast area of imagery and reality.'

            },

            {

                title: 'Artist',
                artist_image: '/images/Art Image/artist_image.jpg',
                artist_image_alttext: 'Artist',
                artist_name: 'Bhabatosh Sutar',
                artist_details: 'Born in 1974. Graduated from the Government College of Art & Craft, Kolkata in 2000. He delved into the genre of Durga Puja art and has received critical acclaim for his creative talent. He has created a body of ephemeral art for more than two decades which have crossed barriers and boundaries of public art and moved millions of people through his experiential design. He is one of the key members of Chanderhaat collective which is involved in the process of investigating, exploring and living in the surrounding of Sarsuna, Kolkata which is an unique blend of urban, moffusil and rural. It is a habitation which is in itself an universe of migratory stories and revelations.'

            },

        ]

    },

    // Advetisement
    {

        id: 3,
        page_name: 'Advertisement',
        page_hero_image: '/images/Advetisement/advetisement_image.jpg',
        page_hero_image_alttext: 'Advertisement',
        advertisement_details: [

            {

                title: 'Advertisement',
                para: 'For corporate and business houses Tala Prattoy is now a potential and prospective arena for eye catching innovative branding and advertisement. Creative and customised brand promotion and brand establishment is the core area of our advertisement.'

            },

            {

                first_button_text: 'Proposal of Durga Puja',
                second_button_text: 'Proposal of Music in Park',
                video_link: 'https://www.youtube.com/embed/zGbTFiA7irY?si=SUE2_r-l44dcQ1VR',
                video_title: 'YouTube video player'

            }

        ]

    }

];

export default plan_art_advertisement_api;